Feature: Flexible JSON assert configuration
  ...
  Rule: It must be possible to add extra elements in JSON response without breaking the tests
  ...
  Rule: It must be possible to ignore extra fields in JSON response
  ...
  Rule: It must be possible to combine some different JSON comparison configurations.
    @bdd_lib_json_ignore_array_order
    @bdd_lib_json_ignore_new_array_elements
