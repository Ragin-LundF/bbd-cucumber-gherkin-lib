Feature: Authorization with PATCH tests
  ...
  Scenario: Authorized PATCH call with direct given API path and dynamic path elements from context and body from file is successful
    Given that the context contains the key "apiPath" with the value "authorized"
    When executing an authorized PATCH call to "/api/v1/${apiPath}" with the body from file "requests/request.json"
    Then I ensure that the status code of the response is 200
    And I ensure that the body of the response is equal to the file "responses/response_authorized.json"
