...
open class BaseCucumberCore(
    ...
    open fun setDefaultBearerToken(defaultBearerToken: String?) {
        ...
        ScenarioStateContext.bearerToken = if (ScenarioStateContext.bearerToken.isNullOrEmpty()) {
            defaultBearerToken
        } else {
            ScenarioStateContext.bearerToken
        }
    }
    ...
    open fun getFilePath(path: String): String {
        return if (path.startsWith(prefix = KEYWORD_ABSOLUTE_PATH)) {
            ...
            ScenarioStateContext.fileBasePath + path
        }
    }
